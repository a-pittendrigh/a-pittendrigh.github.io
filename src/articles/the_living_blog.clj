...
Content will be ordered in descending date of publication (newest to oldest) ."]
   [:p "I want the blog to integrate with other platforms, to look beautiful, to be elegant in code and in how
it
looks.
I want the blog to be easy to index, so that any helpful things I find are freely available to the
public
and easy to find."]
   [:p "The blog is using GitHub pages for now, but I feel like experimenting with server-side rending in
Clojure.
Perhaps it's best to just experiment at work and blog here, since switching to a served blog will
introduce
many hurdles."]
   [:p "First, I want to switch the blog to "
    [:a {:href "https://github.com/weavejester/hiccup"} "Hiccup"]
    " , because while HTML is great for content markup, it's not as nice for developers as Hiccup is."]
   [:p "Source at "
